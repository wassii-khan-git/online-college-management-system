...
if (!isset($_SESSION['student_insert_check']) || $_SESSION['student_insert_check'] !== true) {
...
$email = $_SESSION['student_insert_check_email'];
				...
				if (file_exists($file_dir . $hash_image_exact)) {
					...
					if ($file_size > 1000000) {
						...
						if (in_array($file_extension_lower_case, $image_array)) {
							...
							$query = "UPDATE `students` SET `image` = '$hash_image_exact' WHERE `email` = '$email' ";
								...
								if (isset($_SESSION['logged']) && $_SESSION['logged'] === true) {
									...
									'
										<script>
											...
											setTimeout(function(){
												window.location.href="all-students-teacher.php";
											} , 1000);
